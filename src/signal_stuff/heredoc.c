...
/*   Updated: 2025/06/18 01:47:10 by wweerasi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	heredoc_child(t_msh *msh, t_redir *redir, int *pipefd)
{
	...
	if (!line && !g_sig)//added g_sig here so it won't complain about eof when sigint given
...
void	get_here_doc(t_msh *msh, t_redir *redir, int *hdoc_fd)
{
	...
	if (pipe(pipefd) == -1)
		return (msh_error(msh, (ERRNO | LOG | CLEAN) << 8 | 1, ERR_SYSFUNC, "pipe"));
	signal(SIGINT, SIG_IGN);
	pid = fork();
	if (pid == -1)
		return (msh_error(msh, (ERRNO | LOG | CLEAN) << 8 | 1, ERR_SYSFUNC, "fork"));
	if (pid == 0)
